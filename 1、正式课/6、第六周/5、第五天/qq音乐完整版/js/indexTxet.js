(function () {
    ...
    function computedTime() {
        ...
        let cur = formate(curTime); // '02:35'  // ['02','35']
        if (curTime >= duration) {
            clearInterval(timer);
            $('#musicBtn').removeClass('select');
            return;
        }
        $('#left').html(cur); // 把当前的时间渲染到页面
        $('.lineBg').css('width', curTime / duration * 100 + '%'); // 设置进度条

        // 让对应的歌词显示颜色


        let min = cur.split(':')[0]; // 获取当前正在运行的分钟
        let sec = cur.split(':')[1]; //获取当前正在运行的秒
        ...
        for (var i = 0; i < allP.length; i++) {
            ...
            // 如果当前的时间和p标签的行内的时间对上，那就说明正在唱的就是这句歌词
            if (min == pMin && sec == pSec) {
                flag++;
                // 让歌词的前四句位置不动，从第五句开始移动位置
                if (flag > 4) {
                    curT -= 0.84;
